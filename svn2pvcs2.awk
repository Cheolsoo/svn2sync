...
}
{
	...
	if (length(fileName)>1) {
		...
		#-----		
		print "echo VCSDir=\"v:/"  pdb "/archives/SRC/chess/" unixDirName "\" >> vcs.cfg"
		...
		# v:/test/archives/SRC/chess 구조
		#-----
		print "pcli IA -qw -prv:/" pdb " -pp\"" PPATH "\" -id" PCLI_ID " \"v:/" pdb "/archives/SRC/chess/" dirName2 fileName "-arc\""
		print "\n"
		...
		print "if not !errorlevel! == 0 ("
			...
			for(i=NF; i>1; i--) {
				...
				if ($i == "chess") {
					...
					printf ("  pcli GAL -id%s -pr\"v:/%s\" -pp\"/SRC/chess", PCLI_ID, pdb)					
				}
				...
				for (k=j; k < maxi; k++) {
					...
					printf ("      pcli CP -id%s -prv:/%s -pp\"/SRC/chess", PCLI_ID, pdb)
